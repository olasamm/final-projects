...
const LecturerSignup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      console.log('LecturerSignup.jsx: API response:', res.data);

      if (res.status === 200 || res.status === 201) {
        localStorage.setItem('lecturerId', res.data.id);
        localStorage.setItem('lecturerName', res.data.name);
        setMessage('Lecturer Created Successfully');
        setMessageType('success');
        setTimeout(() => navigate('/lecturerSignin'), 2000);
      }
    } catch (error) {
      console.log('LecturerSignup.jsx: Signup error:', error.response ? error.response.data : error.message);
      if (error.response) {
